...
from scene.cameras import Camera, AvatarCam
import numpy as np
...
def loadAvatarCam(args, id, cam_info):
    
    if hasattr(cam_info, 'FovX'):
        return AvatarCam(colmap_id=cam_info.uid, R=cam_info.R, T=cam_info.T, dir=cam_info.dir, region=cam_info.region,
                        FoVx=cam_info.FovX, FoVy=cam_info.FovY, w=cam_info.width, h=cam_info.height,
                        uid=id, data_device=args.device)
    else:
        return AvatarCam(colmap_id=cam_info.uid, R=cam_info.R, T=cam_info.T, dir=cam_info.dir, region=cam_info.region,
